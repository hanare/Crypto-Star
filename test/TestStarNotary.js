...
it('can add the star name and star symbol properly', async () => {
    ...
    assert.equal(await instance.name.call() ,"Cryto Star")
    assert.equal(await instance.symbol.call(),"CS");
...
it('lets a user transfer a star', async () => {
    ...
    let tokenId = 13;
    let to1 ='0x66532082212643ef736b0ad5381cc1ad2d071813';
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', tokenId, { from: accounts[0] })
...
it('get star name ', async () => {

    let tokenId = 12;

    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', tokenId, { from: accounts[0] })
    let star = await instance.lookUptokenIdToStarInfo(tokenId)//, {from: accounts[0]})
